...
Процедура СоздатьИБ(
    ...
    ВыполнитьСозданиеИБ(Локаль, СмещениеДат);
...
Процедура СоздатьИБИзФайлаВыгрузки(
    ...
    ВыполнитьСозданиеИБ(Локаль, СмещениеДат, "--restore", ПутьКФайлуВыгрузки);
КонецПроцедуры
...
Процедура СоздатьИБИзФайлаКонфигурации(
    ...
    ВыполнитьСозданиеИБ(Локаль, СмещениеДат, "--load", ПутьКФайлуКонфигурации);
КонецПроцедуры
...
Процедура СоздатьИБИзФайловКонфигурации(
    ...
    ВыполнитьСозданиеИБ(Локаль, СмещениеДат, "--import", ПутьККаталогуВыгрузки);
КонецПроцедуры
...
Функция ВыполнитьКоманду(УстановленныйКодВозврата = 0)
	...
	// При ошибке аутентификации ibcmd запрашивает пароль в консоли: без ввода команда зависает
	Если ПараметрыЗапуска.Найти("<") = Неопределено Тогда
		ПараметрыЗапуска.Добавить("<");
		ПараметрыЗапуска.Добавить(?(ЭтоWindows, "NUL", "/dev/null"));
	КонецЕсли;
	...
	Если УспешныеКодыВозврата.Найти(КодВозврата) = Неопределено Тогда
		ОжидаемыеКоды = "";
		Для Каждого Код Из УспешныеКодыВозврата Цикл
			ОжидаемыеКоды = ОжидаемыеКоды + ?(ОжидаемыеКоды = "", "", ", ") + Код;
		КонецЦикла;
...
Процедура ДобавитьСекретныйПараметр(МассивПараметров, Знач ИмяПараметра, Знач ИмяПеременнойСреды, Знач Значение)
    ...
    Лог.Отладка("Значение параметра %1 передается через переменную среды %2", ИмяПараметра, ИмяПеременнойСреды);

    ПеременныеСредыКоманды.Вставить(ИмяПеременнойСреды, Значение);
    Подстановка = ОбернутьВКавычки(СлужебныйIbcmdRunner.ШаблонПеременнойСреды(ИмяПеременнойСреды, ЭтоWindows));
    МассивПараметров.Добавить(СтрШаблон("%1=%2", ИмяПараметра, Подстановка));

КонецПроцедуры

// Выполнить команду создания информационной базы.
