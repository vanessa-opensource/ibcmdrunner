...
Процедура ПроверитьНомерПорта(Знач НомерПорта)
	...
	ТекстОшибки = СтрШаблон("Номер порта должен быть в диапазоне от 1000 до 65535, получено: %1", НомерПорта);
	Ожидаем.Что(НомерПорта).ИмеетТип("Число");
	Ожидаем.Что(НомерПорта, ТекстОшибки).Больше(1000); // BSLLS:MagicNumber-off
	Ожидаем.Что(НомерПорта, ТекстОшибки).Меньше(65535); // BSLLS:MagicNumber-off
