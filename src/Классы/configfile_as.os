...
Процедура СоздатьКонфигурационныйФайл(ПутьКФайлу) Экспорт
	...
	ЗаписатьСекциюВерхнегоУровня(Конфиг, "server", ПараметрыСервера);
	ЗаписатьПараметрыБазыДанных(Конфиг);
	ЗаписатьСекциюВерхнегоУровня(Конфиг, "infobase", ПараметрыИБ);
	ЗаписатьПараметрыВебДоступа(Конфиг);
	ЗаписатьСекциюВерхнегоУровня(Конфиг, "gates", ПараметрыСпособовДоступаКСерверу);
	ЗаписатьСекциюВерхнегоУровня(Конфиг, "features", ПараметрыФункциональностиСервера);
	ЗаписатьСекциюВерхнегоУровня(Конфиг, "debug", ПараметрыОтладкиСервера);

	Конфиг.Закрыть();

	Лог.Отладка("Конфигурационный файл %1 записан", ПутьКФайлу);
...
Процедура ЗаполнитьСекцию(КонфигФайл, Параметры, Уровень = 1)
	...
	Для Каждого Параметр Из Параметры Цикл
		...
		ИначеЕсли ТипЗнчПараметра = Тип("Число") ИЛИ
		...
		Иначе
			ВызватьИсключение СтрШаблон(
				"Значение параметра '%1' имеет неизвестный тип: %2. Допустимые типы: Число, Строка, Булево, Соответствие.",
				Параметр.Ключ, ТипЗнчПараметра);
		КонецЕсли;
...
Функция БулевоВСтроку(Значение)
	Возврат СлужебныйIbcmdRunner.БулевоВСтрокуTrueFalse(Значение);
КонецФункции

// Записать секцию верхнего уровня конфигурационного файла,
// если параметры секции заполнены
// Параметры:
//  Конфиг - ЗаписьТекста - файл конфигурации
//  ИмяСекции - Строка - имя секции верхнего уровня (без двоеточия)
//  Параметры - Соответствие - параметры секции
Процедура ЗаписатьСекциюВерхнегоУровня(Конфиг, Знач ИмяСекции, Знач Параметры)

	Если Параметры.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;

	Лог.Отладка("Запись секции '%1' конфигурационного файла", ИмяСекции);

	Конфиг.ЗаписатьСтроку(ИмяСекции + ":");
	ЗаполнитьСекцию(Конфиг, Параметры);
...
Процедура ЗаписатьПараметрыБазыДанных(Конфиг)
	...
	Лог.Отладка("Запись секции 'database' конфигурационного файла");

	Конфиг.ЗаписатьСтроку("database:");

	ЗаполнитьСекцию(Конфиг, ПараметрыСервернойБазыДанных);
	ЗаполнитьСекцию(Конфиг, ПараметрыФайловойБазыДанных);
...
Процедура ЗаписатьПараметрыВебДоступа(Конфиг)
	...
	ПропуститьСекциюHTTP = ПараметрыХТТП.Количество() = 0
		...
		И ПараметрыВеб_Приложение.Количество() = 0
	...
	Лог.Отладка("Запись секции 'http' конфигурационного файла");
	...
	ЗаписатьПодсекциюВеб(Конфиг, "application", ПараметрыВеб_Приложение);
	ЗаписатьПараметрыВеб_ПубликацияOData(Конфиг);
	ЗаписатьПараметрыВеб_Сервисов(Конфиг, "web-services", ПараметрыВебСервисовОбщие, мсвПараметрыВебСервисов);
	ЗаписатьПараметрыВеб_Сервисов(Конфиг, "http-services", ПараметрыХТТПСервисовОбщие, мсвПараметрыХТТПСервисов);
	ЗаписатьПодсекциюВеб(Конфиг, "pubdst", ПараметрыПубликацииДистрибутивов);
...
Процедура ЗаписатьПараметрыВеб_ПубликацияOData(Конфиг)
	...
	КонецЕсли;
	Значение = ПараметрыВеб_ПубликацияOData.Получить("reuse-sessions");
	Если Значение <> Неопределено Тогда
		Конфиг.ЗаписатьСтроку("    reuse-sessions:");
		ЗаполнитьСекцию(Конфиг, Значение, 3);
	КонецЕсли;

КонецПроцедуры

// Записать подсекцию секции http с общими параметрами сервисов
// и массивом описаний отдельных сервисов
// Параметры:
//  Конфиг - ЗаписьТекста - файл конфигурации
//  ИмяПодсекции - Строка - имя подсекции ("web-services" или "http-services")
...
Процедура ЗаписатьПараметрыВеб_Сервисов(Конфиг, Знач ИмяПодсекции, Знач ОбщиеПараметры, Знач МассивСервисов)

	Если ОбщиеПараметры.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;

	Конфиг.ЗаписатьСтроку(СтрШаблон("  %1:", ИмяПодсекции));
	ЗаполнитьСекцию(Конфиг, ОбщиеПараметры, 2);
	Если МассивСервисов.Количество() > 0 Тогда
		Конфиг.ЗаписатьСтроку("    service:");
		Для Каждого ПараметрыСервиса Из МассивСервисов Цикл
			Конфиг.ЗаписатьСтроку("    -");
			ЗаполнитьСекцию(Конфиг, ПараметрыСервиса, 3);
		КонецЦикла;
	КонецЕсли;

КонецПроцедуры

// Записать простую подсекцию секции http
// Параметры:
//  Конфиг - ЗаписьТекста - файл конфигурации
//  ИмяПодсекции - Строка - имя подсекции
//  Параметры - Соответствие - параметры подсекции
Процедура ЗаписатьПодсекциюВеб(Конфиг, Знач ИмяПодсекции, Знач Параметры)

	Если Параметры.Количество() > 0 Тогда
		Конфиг.ЗаписатьСтроку(СтрШаблон("  %1:", ИмяПодсекции));
		ЗаполнитьСекцию(Конфиг, Параметры, 2);
	КонецЕсли;
