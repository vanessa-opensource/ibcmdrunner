...
Функция ПолучитьСписокТестов(Тестирование) Экспорт
	...
	СписокТестов.Добавить("ТестДолжен_ПередатьВсеВыбранныеФайлы");
	СписокТестов.Добавить("ТестДолжен_ПередатьПараметрыАвторизацииСоСпецсимволами");
	СписокТестов.Добавить("ТестДолжен_ЗавершитьсяОшибкойПриЗапросеПароля");
...
Процедура ТестДолжен_ИзменитьСвойстваРасширения() Экспорт
	...
	УправлениеИБ.ИзменитьСвойстваРасширения("СуперРасширение", Ложь, Истина, "", Истина, Ложь, Истина);
	Результат = УправлениеИБ.ИнформацияОРасширении("СуперРасширение");
	Сообщить(Результат);
...
Процедура ТестДолжен_ПередатьВсеВыбранныеФайлы() Экспорт

	// Дано
	УстановитьПараметрыВременнойИБ();
	КаталогВыгрузки = ПолныйПутьШаблонаДанных("1.0", "files");
	УправлениеИБ.СоздатьИБИзФайловКонфигурации(КаталогВыгрузки);
	УправлениеИБ.УстановитьПараметрыАвторизацииИБ(""); // в ИБ из файлов нет пользователей

	ВыбранныеФайлы = Новый Массив;
	ВыбранныеФайлы.Добавить("Missing.xml");
	ВыбранныеФайлы.Добавить("Configuration.xml");

	ПараметрыМетода = Новый Массив;
	ПараметрыМетода.Добавить(КаталогВыгрузки);
	ПараметрыМетода.Добавить(ВыбранныеФайлы);
	ПараметрыМетода.Добавить("");
	ПараметрыМетода.Добавить(Истина);

	// Тогда: первый файл списка дошёл до ibcmd, и она сообщила, что его нет
	Ожидаем.Что(УправлениеИБ)
		.Метод("ЗагрузитьВыбранныеФайлыКонфигурации", ПараметрыМетода)
		.ВыбрасываетИсключение("Missing.xml");
...
Процедура ТестДолжен_ПередатьПараметрыАвторизацииСоСпецсимволами() Экспорт

	// Когда
	Пользователь = "user & (x) ^ %PATH% $HOME";
	ТекстОшибки = ТекстОшибкиАутентификации(Пользователь, "pass & word");

	// Тогда: ibcmd называет пользователя в ошибке, значит имя дошло целиком
	Утверждения.ПроверитьИстину(СтрНайти(ТекстОшибки, Пользователь) > 0,
		"Имя пользователя должно дойти до ibcmd целиком: " + ТекстОшибки);

КонецПроцедуры

Процедура ТестДолжен_ЗавершитьсяОшибкойПриЗапросеПароля() Экспорт

	// Когда: ibcmd запрашивает пароль в консоли
	ТекстОшибки = ТекстОшибкиАутентификации("admin", "admin");

	// Тогда
	Утверждения.ПроверитьИстину(ЗначениеЗаполнено(ТекстОшибки),
		"Команда должна завершиться ошибкой, а не ждать ввода пароля");

КонецПроцедуры

Функция ТекстОшибкиАутентификации(Пользователь, Пароль)

	// В пустой ИБ нет пользователей: аутентификация заведомо не проходит
	УстановитьПараметрыВременнойИБ();
	УправлениеИБ.СоздатьИБ();
	...
	ТекстОшибки = "";
	Попытка
		УправлениеИБ.ВыгрузитьКонфигурациюВФайл(ОбъединитьПути(ВременныйКаталог, "1Cv8.cf"));
	Исключение
